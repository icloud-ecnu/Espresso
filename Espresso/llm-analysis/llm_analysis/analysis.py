...
class LLMAnalysis:
    ...
    def get_memory_weight_per_layer(
        ...
        # print(f'memory_weight_attn_per_layer: {_num_to_string(memory_weight_attn_per_layer)}B, memory_weight_mlp_per_layer: {_num_to_string(memory_weight_mlp_per_layer)}B, memory_weight_layernorm_per_layer: {_num_to_string(memory_weight_layernorm_per_layer)}B')
        if return_breakdown:
    ...
    def detail_info(
        ...
        # print("self.model_config.num_layers = ",self.model_config.num_layers)
        if seq_len is None:
            assert (
                self.model_config.max_seq_len
                is not None), "seq_len must be set if max_seq_len is not set"
        ...
        weight_memory_embedding = self.get_memory_embedding(ds_zero)
        # 计算权重
        weight_memory_layer = self.get_memory_weight_per_layer(ds_zero,return_breakdown=False)
        weight_memory_last_layernorm = self.get_memory_weight_last_layernorm(
            ds_zero)
        # 计算优化器状态和梯度 直接用即可
        optimizer_state_memory_per_layer, gradient_memory_per_layer = self.get_memory_optimizer_state_and_gradient_per_layer(
        ...
        embedding_memory = 0
        if first_embedding:
            embedding_memory = 0.5 * 1024 ** 3 + weight_memory_embedding + optimizer_state_memory_embedding + gradient_memory_embedding
        if last_embedding:
            embedding_memory = 0.5 * 1024 ** 3 + weight_memory_embedding + optimizer_state_memory_embedding + gradient_memory_embedding + activation_memory_embedding_output_per_layer
        ...
        # if last_embedding:
        #     total_memory = 0.8829*total_memory+6.6861*1024**3
        # else:
        # total_memory = 1.03835*total_memory+8.26135*1024**3 + 1.2*1024**3 # 2000
        # total_memory = 0.9336*total_memory+8.0341*1024**3 + 1.2*1024**3 # 1700 + 碎片
        # total_memory = 0.918975*total_memory+7.516225*1024**3 #+ 1.2*1024**3 # 1600 + 碎片
        # total_memory = 0.8951*total_memory+ 6.1672*1024**3 #+ 1*1024**3 # 1400 + 碎片
        batch_8_fit = {
            # "llama_1.3b":{
            #     1800:{
            #         'a':1.2043,
        ...
        def estimate_ab(seqlen, known_points):
            if seqlen in known_points:
                return known_points[seqlen]['a'], known_points[seqlen]['b']
            x1, x2 = known_points.keys()
            y1_a, y1_b = known_points[x1]['a'], known_points[x1]['b']
            y2_a, y2_b = known_points[x2]['a'], known_points[x2]['b']
            slope_a = (y2_a - y1_a) / (x2 - x1)
            slope_b = (y2_b - y1_b) / (x2 - x1)
            a_intercept = y1_a - slope_a * x1
            b_intercept = y1_b - slope_b * x1
            a_estimated = slope_a * seqlen + a_intercept
            b_estimated = slope_b * seqlen + b_intercept
            return a_estimated, b_estimated
        
        # print(model_name,total_memory)
        if "ViT" in  model_name :
            a_est, b_est = estimate_ab(batch_size_per_gpu,vit)
        elif "llama" in model_name:
            a_est, b_est = estimate_ab(seq_len,llama3b_8)
        else:
            a_est,b_est = 1,0
        # total_memory = a_est * total_memory + (b_est + 0)*1024**3
        # total_memory = 1.2464 * total_memory + 2.3029 * 1024 ** 3
        total_memory = a_est * total_memory + (b_est + 0)*1024**3
        
        memory_left = (self.gpu_config.mem_per_GPU_in_GB * 1024**3 - total_memory)
        logger.info(f"total_memory = {_num_to_string(total_memory)}")

        # print(batch_size_per_gpu,seq_len)
        # latency
        (
            latency_fwd_per_layer,
            breakdown_per_layer,
        ...
        # print(f"bandwith = {self.get_inter_node_bandwidth()}")
        comm_latency = seq_len * batch_size_per_gpu * self.model_config.hidden_dim * self.dtype_config.activation_bits / (self.get_inter_node_bandwidth() * 1000**3)
        ...
        # flop
        num_flops_fwd = self.get_num_flops_fwd_per_layer(batch_size_per_gpu, seq_len)
        try:
...
def train(
    ...
    if model_layer:
        model_config.num_layers = model_layer
    gpu_config = get_gpu_config_by_name(gpu_name)
    dtype_config = get_dtype_config_by_name(dtype_name)
    parallel_config = ParallelismConfig(
        tp_size=tp_size,
