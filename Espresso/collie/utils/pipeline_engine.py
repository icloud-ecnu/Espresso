...
class ColliePipelineEngine(PipelineEngine):
    def __init__(self, has_bool_tensors=False, *args, **kwargs):
        # in  /home/ecnu/anaconda3/envs/echo/lib/python3.8/site-packages/deepspeed/runtime/engine.py(184)__init__()
        ...
        print(f"self.bfloat16_enabled() = {self.bfloat16_enabled()}")
        self.using_bf16_optimizer = self.bfloat16_enabled()
    ...
    def _exec_send_grads(self, buffer_id):
        ...
        inputs = self.pipe_buffers['inputs'][buffer_id] # backward buffer_id

        # Partition the gradient
        # print(f"_exec_send_grads self.prev_stage = {self.prev_stage} rank = {self.global_rank}")
        if self.is_grad_partitioned:
            part = None
            _grad_key = None
    ...
    def _exec_recv_activations(self, buffer_id):
        ...
        # print(f"_exec_recv_activations self.prev_stage = {self.prev_stage} rank = {self.global_rank}")
        recvd = None
        ...
        # print(f"self.pipe_buffers['inputs'][buffer_id] len = {len(self.pipe_buffers['inputs'])} buffer_id = {buffer_id}")
    ...
    def _exec_recv_grads(self, buffer_id):
        ...
        # print(f"_exec_recv_grads self.prev_stage = {self.prev_stage} rank = {self.global_rank}")
        outputs = self.pipe_buffers['outputs'][buffer_id]
        ...
        if isinstance(inputs, torch.Tensor):
            ...
            for k, t in inputs.items():
                if t.grad is not None:
                    # print(f"rank = {self.global_rank} _exec_forward_pass inputs.grad")
                    t.grad.data.zero_()
