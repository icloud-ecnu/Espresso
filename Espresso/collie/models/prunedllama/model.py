...
class PrunedLlamaLayer(nn.Module):
    ...
    def record_time_and_memory(self,start_time, details):
        cur_time = time.time()
        elapsed_time = cur_time - start_time
        details.append([elapsed_time,self.layer_idx,start_time,cur_time,env.rank])
    ...
    def _forward(self, 
        ...
        self.record_time_and_memory(start_time, self.details)
        if len(self.details)>100:
            with open(f"{os.environ.get('FILE_NAME')}/{env.rank}.csv", 'a+', newline='') as file:
...
class PrunedLlamaModel(nn.Module):
    def __init__(self, config: CollieConfig):
        ...
        self.layers = nn.ModuleList(
            [PrunedLlamaLayer(self.config) for _ in range(self.config.num_hidden_layers)])
    ...
    def pipeline_layers(cls, config: CollieConfig):
        ...
        layers = [
            LayerSpec(PrunedLlamaLayer, config,_)
...
class PrunedLlamaForCausalLM(CollieModelForCausalLM):
    ...
    def pipeline_layers(cls, config: CollieConfig):
        ...
        return [
            ("model", PrunedLlamaModel.pipeline_layers(config)),
            ("lm_head", lm_head)
        ]
