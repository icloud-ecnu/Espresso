    ...
    **CoLLiE** 的基础模型。如果要实现新的模型，必须继承该基类。
    ...
    def from_config(cls, config: Union[CollieConfig, str], **kwargs):
        ...
        if config.pp_size == 1:
            ...
            # print("=====model_cls.pipeline_layers(config) ==")
            # print(model_cls.pipeline_layers(config))
            # print("===end===")
                ...
                for name, param in model.named_parameters():
                    ...
                    with ContextManagers(contexts):
                        if param.device == torch.device("meta"):
                            ...
                            if name not in post_init_funcs:
                                init_fun = config.init_method['init_func']
                                param.data = init_fun(torch.zeros_like(param.data), **config.init_method['init_kwargs']).to(config.model_config.torch_dtype).to(param.device)
                            else:
