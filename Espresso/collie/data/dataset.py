...
class CollieDatasetForTraining(Dataset):
    ...
    def __getitem__(self, index) -> Tuple:
        ...
        if self.max_length > 0:
            ...
            input_ids.extend([1 for x in range(self.max_length - len(input_ids))])
            attention_mask.extend([1 for x in range(self.max_length - len(attention_mask))])
            labels.extend([1 for x in range(self.max_length - len(labels))])
    ...
    def from_json(cls,
                  ...
                  tokenizer: Optional[PreTrainedTokenizer] = None,
                  shuffle: bool = False,
                  seed: int = 1024, max_length: int = 2048):
        dataset = cls(dataset=json.loads(FileIODriver.load(
            path, mode="r")), shuffle=shuffle, seed=seed, tokenizer=tokenizer, max_length=max_length)
        return dataset
